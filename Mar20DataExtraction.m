...
for aaaa=1:1
    ...
    for count=j:m
        if(count>d)
            if((app(count,1)<app(count-1,1))||(times(count)-times(count-1)>=0.02))
                phi=[phi;phiappend];
                q=[q;qappend];
                pti=[pti;ptiappend];
                phiappend=[1:360];
                qappend=zeros(1,360);
                ptiappend=zeros(1,360);
            elseif((app(count,1)<app(count-1,1))&&(times(count)-times(count-1)>0.02))
                numb=(times(count)-times(count-1))/0.02;
                for k=1:numb
                    phiappend=[1:360];
                    qappend=zeros(1,360);
                    ptiappend=zeros(1,360);
    ...
    xlswrite(strcat(filename1,'-PQNTIdata.xlsx'),PQN);
end